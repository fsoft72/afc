...
TRY ( ReadArgs * )
	...
	if ( rdargs == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "rdargs", NULL );
	rdargs->magic = AFC_READARGS_MAGIC;	// Set the Magic value

	// Create the NodeMaster for the Template Fields
	if ( ( rdargs->fields = afc_nodemaster_new () ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "fields", NULL );

	// Create the NodeMaster for the string tokens
	if ( ( rdargs->str = afc_nodemaster_new () ) == NULL )	RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "str", NULL );

	// Create the main string Splitter
	if ( ( rdargs->global_split = afc_stringnode_new () ) == NULL )	RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "global_split", NULL );

	// Create the secondary string splitter
	if ( ( rdargs->local_split = afc_stringnode_new () ) == NULL )	RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "local_split", NULL );
...
int _afc_readargs_delete ( ReadArgs * rdargs )
{
	...
	if ( ( res = afc_readargs_clear ( rdargs ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
int afc_readargs_parse ( ReadArgs * rdargs, const char * template, const char * text )
{
	...
	if ( text == NULL ) 		return ( AFC_ERR_NO_ERROR );
	if ( strlen ( text ) == 0 ) 	return ( AFC_ERR_NO_ERROR );
...
int afc_readargs_clear ( ReadArgs * rdargs )
{
	...
	if ( rdargs == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	if ( rdargs->magic != AFC_READARGS_MAGIC ) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );

	if ( rdargs->buffer ) afc_string_delete ( rdargs->buffer );
...
int afc_readargs_parse_cmd_line ( ReadArgs * rdargs, char * template, int argc, char * argv[] )
{
	...
	for ( t = 0; t < argc; t++ )
	{
		...
		if ( do_quotes ) afc_string_add ( s, "\"", 1 );
...
static int afc_readargs_internal_add_template ( ReadArgs * rdargs, char *tok )
{
	...
	if ( data == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NO_MEMORY ) );

	if ( ( res = afc_stringnode_split ( rdargs->local_split, tok, "/" ) ) != AFC_ERR_NO_ERROR ) return ( res );
		...
		if ( ( arg->is_required ) && ( arg->data == NULL ) && ( arg->multi == NULL ) ) 
		{
			if ( ( s = afc_readargs_internal_get_keyword ( rdargs, arg->name, arg->is_switch, arg->is_numeric ) ) == NULL ) 
			{
				...
				if ( arg->is_numeric ) 
				{
					...
					if ( errno ) return ( AFC_LOG ( AFC_LOG_ERROR, AFC_READARGS_ERR_NOT_A_NUMBER, strerror ( errno ), s ) );
...
static void afc_readargs_data_delete ( struct afc_readargs_data *rddata )
{
	...
	if ( rddata == NULL ) return;

	if ( rddata->name ) 	afc_string_delete ( rddata->name );
	if ( rddata->multi ) 	
	{
