...
RegExp *afc_regexp_new()
{
	...
	if (regexp == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "regexp", NULL);
	regexp->magic = AFC_REGEXP_MAGIC;

	if ((regexp->buffer = afc_string_new(AFC_REGEXP_BUFFER)) == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "buffer", NULL);
	if ((regexp->ovector = afc_malloc(sizeof(int) * AFC_REGEXP_MAX_OSIZE)) == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "ovector", NULL);
...
int _afc_regexp_delete(RegExp *regexp)
{
	...
	if ((afc_res = afc_regexp_clear(regexp)) != AFC_ERR_NO_ERROR)
		return (afc_res);
...
int afc_regexp_clear(RegExp *regexp)
{
	...
	if (regexp->magic != AFC_REGEXP_MAGIC)
	...
	if (regexp->pattern)
		afc_free(regexp->pattern);
	if (regexp->hints)
		afc_free(regexp->hints);
...
int afc_regexp_match(RegExp *regexp, const char *str, const int startpos)
{
	if ((str == NULL) || (strlen(str) == 0))
		return (1); // No string provided for the match
	if (regexp->ready == FALSE)
		return (AFC_LOG(AFC_LOG_WARNING, AFC_REGEXP_ERR_NOT_READY, "RegExp not ready. Missing compile()?", NULL));
...
{
	if (regexp->matches <= 0)
		return (AFC_LOG(AFC_LOG_NOTICE, AFC_REGEXP_ERR_NO_MATCH, "No match for current expression", NULL));
	if (pos > regexp->matches)
		return (AFC_LOG(AFC_LOG_NOTICE, AFC_REGEXP_ERR_OUT_OF_BOUNDS, "Queried result out of bounds", NULL));
...
int afc_regexp_get_sub_string(RegExp *regexp, char *dest, int pos)
{
	...
	if ((res = afc_regexp_get_pos(regexp, pos, &coords)) != AFC_ERR_NO_ERROR)
		return (res);
...
int afc_regexp_replace(RegExp *regexp, char *dest, char *str, char *str_re, char *replace, short replace_all)
{
	...
	if ((err = afc_regexp_compile(regexp, str_re)) != AFC_ERR_NO_ERROR)
		return (err);

	if ((buf = afc_string_new(afc_string_max(dest))) == NULL)
	...
	{
		if ((err = afc_regexp_get_pos(regexp, 0, &coords)) != AFC_ERR_NO_ERROR)
			return (err);
		if ((err = afc_regexp_internal_replace(regexp, regexp->buffer)) != AFC_ERR_NO_ERROR)
			return (err);
		...
		if (coords.start > 0)
			afc_string_copy(dest, buf, coords.start);
...
static int afc_regexp_internal_replace(RegExp *regexp, char *replace)
{
	...
	if (tmp_regex == NULL)
		return (AFC_LOG_FAST_INFO(AFC_ERR_NO_MEMORY, "tmp_regex"));
	...
	while ((afc_regexp_match(tmp_regex, replace, 0)) == AFC_ERR_NO_ERROR)
	{
		...
		if (coords.start > 0)
			afc_string_copy(tmp_regex->buffer, replace, coords.start);
