...
enum { AFC_DIRMASTER_ERR_DIR_NOT_FOUND = AFC_DIRMASTER_BASE + 1 };

enum {	AFC_DIRMASTER_TAG_DATE_FORMAT = AFC_DIRMASTER_BASE + 1,
...
/* Size Format */
enum  { SIZEFORMAT_BYTES = 0,
	SIZEFORMAT_HUMAN,			/* Human readable sizes, with 1K corresponding to 1024 bytes */
	SIZEFORMAT_HUMAN_1000 };		/* Human readable sizes, with 1K corresponding to 1000 bytes */
...
#define afc_dirmaster_delete(dm) if ( dm ) { _afc_dirmaster_delete ( dm ); dm = NULL; }
...
#define afc_dirmaster_first(d)		( FileInfo * ) ( d ? afc_array_master_first ( d->am ) : NULL )
#define afc_dirmaster_last(d)		( FileInfo * ) ( d ? afc_array_master_last  ( d->am ) : NULL )
#define afc_dirmaster_prev(d)		( FileInfo * ) ( d ? afc_array_master_prev  ( d->am ) : NULL )
#define afc_dirmaster_item(d,n)		( FileInfo * ) ( d ? afc_array_master_item  ( d->am,n ) : NULL )
#define afc_dirmaster_succ(d)		( FileInfo * ) ( d ? afc_array_master_next  ( d->am ) : NULL )
#define afc_dirmaster_next(d)		( FileInfo * ) ( d ? afc_array_master_next  ( d->am ) : NULL )
#define afc_dirmaster_len(d)		( d ? afc_array_master_len(d->am) : 0 )
#define afc_dirmaster_num_items(d)	( d ? afc_array_master_len(d->am) : 0 )
#define afc_dirmaster_before_first(d)	( d ? afc_array_master_before_first ( d->am ) : 0 )
#define afc_dirmaster_is_empty(d)	( d ? afc_array_master_is_empty ( d->am ) : TRUE )
#define afc_dirmaster_pos(d)		( d ? afc_array_master_pos(d->am) : 0 )
