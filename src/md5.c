...
MD5 *afc_md5_new(void)
{
	...
	if (m == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "md5", NULL);
	m->magic = AFC_MD5_MAGIC;

	if ((m->result = afc_string_new(33)) == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "result", NULL);
...
int _afc_md5_delete(MD5 *m)
{
	...
	if ((afc_res = afc_md5_clear(m)) != AFC_ERR_NO_ERROR)
		return (afc_res);
...
int afc_md5_update(MD5 *m, unsigned char *input, unsigned int input_len)
{
	...
	if ((m->count[0] += ((unsigned int)input_len << 3)) < ((unsigned int)input_len << 3))
		m->count[1]++;
	...
	if (input_len >= part_len)
	{
		...
		for (i = part_len; i + 63 < input_len; i += 64)
			afc_md5_internal_transform(m->state, &input[i]);
