...
TRY ( InetServer * )
	...
	InetServer * is = ( InetServer * ) afc_malloc ( sizeof ( InetServer ) );
  	...
  	if ( is == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "is", NULL );
  	is->magic = AFC_INET_SERVER_MAGIC;
	if ( ( is->hash = afc_hash_master_new () ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "hash", NULL );
...
int _afc_inet_server_delete ( InetServer * is ) 
{
  	...
  	if ( ( afc_res = afc_inet_server_clear ( is ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );
...
{
  	if ( is == NULL ) return ( AFC_ERR_NULL_POINTER );
 
  	if ( is->magic != AFC_INET_SERVER_MAGIC ) return ( AFC_ERR_INVALID_POINTER );
		...
		if ( FD_ISSET ( i, & is->read_fds ) )
		{
			if ( i == is->listener )	// New connection coming :-)
			{
				...
				if ( ( is->newfd = accept ( is->listener,  ( struct sockaddr * ) & is->remoteaddr, & addrlen ) ) == -1 )
				{
					...
					if ( is->newfd > is->fdmax ) is->fdmax =is->newfd;	// keep track of the maximum fd
				}
			} else {
				...
				if ( ( nbytes = recv ( i, data->buf, afc_string_max ( data->buf ), 0 ) ) <= 0 )
				{
					...
					if ( data->cb_receive ) is->cb_receive ( is, data );
				}
			}
		}
	}
...
int afc_inet_server_send ( InetServer * is, InetConnData * data, char * str )
{
	if ( FD_ISSET ( data->fd, & is->master ) )
	{
		...
		if ( data->fd == is->listener ) return ( AFC_ERR_NO_ERROR );
...
{
	if ( data->cb_close != NULL ) data->cb_close ( is, data );

	close ( data->fd );
	FD_CLR ( data->fd, & is->master );

	if ( data->buf ) afc_string_delete ( data->buf );
...
static InetConnData * afc_inet_server_create_conn_data ( InetServer * is, int fd )
{
	...
	if ( ( is->cb_connect ) != NULL )  is->cb_connect ( is, data );
