...
static TreeNode *afc_tree_int_node_delete ( TreeNode * n )
{
	...
	if (tree->first == n) tree->first = next;
	if (tree->last == n) tree->last = n->list_prev;

	if (n->list_prev) n->list_prev->list_next = n->list_next;
	if (n->list_next) n->list_next->list_prev = n->list_prev;

	if (parent) {
		if (parent->last_child == n) { parent->last_child = n->l_sibling; }
		if (parent->child == n) { parent->child = n->r_sibling; }
	}

	if (n->l_sibling) { n->l_sibling->r_sibling = n->r_sibling; }
	if (n->r_sibling) { n->r_sibling->l_sibling = n->l_sibling; }

	if ( tree->freenode ) tree->freenode ( n->val );
	afc_free ( n );
...
TRY ( Tree * )
	...
	if ( t == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "tree", NULL );
...
int _afc_tree_delete ( Tree * t )
{
	...
	if ( ( res = afc_tree_clear ( t ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
int afc_tree_clear ( Tree * tree )
{
	...
	if ( tree == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	if ( tree->magic != AFC_TREE_MAGIC ) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );
...
TreeNode *afc_subtree_insert_sibling ( TreeNode * brother, void * val )
{
	...
	if ( tree->magic != AFC_TREE_MAGIC ) return ( NULL );

	TreeNode * sibling = afc_tree_int_node_new ( tree, val );

	sibling->l_sibling = brother;
	if (brother->r_sibling) { sibling->r_sibling = brother->r_sibling; brother->r_sibling->l_sibling = sibling; }
	else if (brother->parent) { brother->parent->last_child = sibling; }
...
int afc_tree_node_preorder_visit ( TreeNode * parent, int ( *visitor ) ( TreeNode *  ) )
{
	...
	if (!parent) return AFC_ERR_INVALID_POINTER;
...
int afc_tree_traverse ( Tree * t, int mode, int ( *visitor ) ( TreeNode *  ) )
{
	...
	if ( t == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	if ( t->magic != AFC_TREE_MAGIC ) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );
...
{
	if ( n == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );

	switch ( mode )
	{
		case AFC_TREE_MODE_PREORDER:  return ( afc_tree_node_preorder_visit ( n, visitor ) );
		case AFC_TREE_MODE_POSTORDER: return ( afc_tree_node_postorder_visit ( n, visitor ) );
	}
...
{
	if (subtree == NULL) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );

	if (subtree->parent) {
		if (subtree->parent->last_child == subtree) subtree->parent->last_child = subtree->l_sibling;
		if (subtree->parent->child == subtree) subtree->parent->child = subtree->r_sibling;
	}
