...
int _afc_date_handler_delete(DateHandler *dh)
{
	...
	if ((afc_res = afc_date_handler_clear(dh)) != AFC_ERR_NO_ERROR)
		return (afc_res);
...
int afc_date_handler_clear(DateHandler *dh)
{
	...
	if (dh->magic != AFC_DATE_HANDLER_MAGIC)
...
int afc_date_handler_set(DateHandler *dh, int year, int month, int day)
{
	...
	if ((res = afc_date_handler_is_valid(dh, year, month, day)) != AFC_ERR_NO_ERROR)
		return (res);
...
int afc_date_handler_is_valid(DateHandler *dh, int year, int month, int day)
{
	...
	if (year < 0 ||
		...
		return (AFC_LOG(AFC_LOG_WARNING, AFC_DATE_HANDLER_ERR_INVALID_DATE, "Invalid date", NULL));
