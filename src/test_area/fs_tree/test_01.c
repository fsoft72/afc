...
int import_list ( FSTree * fst, FILE * fh, char * buf )
{
	...
	if ( sn == NULL ) return ( AFC_ERR_NO_MEMORY );
	...
	while ( afc_string_fget ( buf, fh ) )
	{
		...
		name 		= afc_stringnode_item ( sn, 5 );
		info->size	= atoi ( afc_stringnode_item ( sn, 3 ) );
		info->perm	= afc_string_dup ( afc_stringnode_item ( sn, 0 ) );

		afc_string_copy ( buf, afc_stringnode_item ( sn, 1 ), ALL );
		

		if ( ( x = index ( buf, '/' ) ) != NULL  )
		{
			x[0] = '\0';
			info->uname = afc_string_dup ( buf );
			info->gname = afc_string_dup ( x+1 );
		} 
...
void dump_dir ( FSTree * fst, char * path )
{
	...
	data = afc_array_master_first ( am );
	while ( data )
	{
