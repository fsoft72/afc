...
int main ()
{
	...
	d = afc_array_master_new ();
	afc_array_master_set_clear_func ( d, _clean_func );

	for ( t = 0; t < 100; t ++ )
	{
		if ( ! ( t % 300 ) ) printf ( "Pos: %d\n", t );

		sprintf ( buf, "key%d", t );
		afc_array_master_add_tail ( d, afc_string_dup ( buf ) );
	}

	// afc_array_master_clear ( d );

	afc_array_master_delete ( d );
	afc_delete ( afc );
