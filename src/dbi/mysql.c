...
void _fini ( void )
{
	if ( __mysql_client ) dlclose ( __mysql_client );
}
#endif
...
static int mysql_method_fetch_row ( DynamicClass * dc )
{
	...
	if ( mysql_int_next_row ( dc, db ) != AFC_ERR_NO_ERROR ) return ( res );
...
static int mysql_int_next_row ( DynamicClass * dc, DBMysql * db )
{
	if ( db->result == NULL ) return ( AFC_LOG ( AFC_LOG_ERROR, AFC_DBI_MANAGER_ERR_NO_RESULT_SET, "No result set defined.", NULL ) );
...
static int mysql_int_free ( DynamicClass * dc, DBMysql * db )
{
	...
	if ( db->result ) mysql_free_result ( db->result );
