...
{
	if ( fo == NULL ) return ( AFC_ERR_NO_ERROR );
	if ( fo->foc ) afc_fileops_internal_copy_delete ( fo->foc );
...
int afc_fileops_set_tag ( FileOperations * fo, int attr, void * val )
{
	...
	switch ( attr )
	{
		...
		case AFC_FILEOPS_TAG_BUFFER:
			...
			if ( ( fo->foc->buffer = ( char * ) afc_malloc ( ( int ) ( long ) val ) ) == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NO_MEMORY ) );
...
int afc_fileops_exists_full (FileOperations * fo, char * dir, char * fname)
{
	...
	if ( ( dir == NULL ) || ( fname == NULL ) ) return ( AFC_ERR_NULL_POINTER );

	if ( ( path = afc_string_new ( strlen ( dir ) + strlen ( fname ) + 10 ) ) == NULL ) return ( AFC_ERR_NO_MEMORY );
...
{
	if ( fo->update_funct ) fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_FILENAME, fname, fo->update_info );
	...
	if ( ( chown ( fname, uid, gid ) ) == -1 )
	{
		...
		if ( fo->block_chown ) return ( AFC_LOG ( AFC_LOG_ERROR, AFC_FILEOPS_ERR_CHOWN, strerror ( errno ), fname ) );
	}
...
int afc_fileops_move ( FileOperations * fo, char * source, char * dest )
{
	...
	if ( errno == EXDEV )
	{
		...
		if ( fo->update_funct ) fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_FILENAME, source, fo->update_info );
...
int afc_fileops_copy ( FileOperations * fo, char * source, char * dest )
{
	...
	if ( fo->foc->buffer == NULL )	return ( AFC_LOG ( AFC_LOG_ERROR, AFC_FILEOPS_COPY_ERR_NO_BUFFER, "No buffer avaible", NULL ) );
	...
	{
		if ( ( err = afc_fileops_mkdir ( fo, fo->foc->dest ) ) != AFC_ERR_NO_ERROR ) return ( err );

	  if ( ( err = afc_fileops_internal_scan_dir ( fo, fo->foc->source, afc_fileops_internal_copy_new_file, afc_fileops_internal_copy_new_dir, NULL, fo->foc->dest ) ) != AFC_ERR_NO_ERROR ) return ( err );
	} else {
...
int afc_fileops_chmod ( FileOperations * fo, char * fname, int mode )
{
	...
	if ( ( chmod ( fname, mode ) ) == -1 )
	{
		...
		if ( fo->block_chmod ) return ( AFC_LOG ( AFC_LOG_ERROR, AFC_FILEOPS_ERR_CHMOD, strerror ( errno ), fname ) );
	}
...
int afc_fileops_utime ( FileOperations * fo, char * fname, struct utimbuf * times )
{
	...
	if ( utime ( fname, times ) == -1 )
	{
		...
		if ( fo->block_utime ) return ( AFC_LOG ( AFC_LOG_NOTICE, AFC_FILEOPS_ERR_UTIME, strerror ( errno ), fname ) );
	}
...
{
	if ( fo->update_funct ) fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_FILENAME, name, fo->update_info );
...
#endif // MINGW	
	{
		if ( ( errno == EEXIST ) && ( fo->block_mkdir_exists == FALSE ) ) return ( AFC_ERR_NO_ERROR );
...
static int afc_fileops_internal_scan_dir ( FileOperations * fo, char * path, int action_file ( FileOperations *,  struct stat *, char *, char *, char *, void *), int action_dir ( FileOperations *, struct stat *,  char *, char *, char *, void * ), int action_end_dir ( FileOperations *, char *, void * ), void * info )
{
	...
	while ( (file=readdir(dir))!=NULL)
	{
...
#endif			
	  {
		...
		{
			if ( action_dir ) 
	      if ( ( err = action_dir ( fo, &descr, fullname, dirname, file->d_name, info ) ) != 0 ) return ( err );

		} else {
			if ( action_file ) 
	if ( ( err = action_file ( fo, &descr, fullname, dirname, file->d_name, info ) ) != 0 ) return ( err );
		}
	}
	...
	if ( action_end_dir ) 
		if ( ( err = action_end_dir ( fo, path, info ) ) != 0 ) return ( err );
...
static int afc_fileops_internal_move_dir ( FileOperations * fo, struct stat * descr, char * fulldir, char * path, char * dirname, void * info )
{
	...
	if ( ( err = afc_fileops_mkdir ( fo, buf ) ) != AFC_ERR_NO_ERROR ) return ( err );
...
static int afc_fileops_internal_del_file ( FileOperations * fo, struct stat * descr, char * fullname, char * path, char * filename, void * info )
{
	...
	if ( fo->update_funct ) fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_FILENAME, buf, fo->update_info );
...
static void afc_fileops_internal_copy_delete ( FOCopy * foc )
{
	...
	if ( foc->buffer ) 	afc_free ( foc->buffer );
...
#ifndef MINGW	
	if ( ( err = afc_fileops_chown ( fo, fo->foc->dest, uid, gid ) ) != AFC_ERR_NO_ERROR ) return ( err );
#endif // MINGW	

	if ( ( err = afc_fileops_chmod ( fo, fo->foc->dest, mode ) ) != AFC_ERR_NO_ERROR ) return ( err );

	times.actime = st->st_atime;
	times.modtime = st->st_mtime;

	if ( ( err = afc_fileops_utime ( fo, fo->foc->dest, &times ) ) != AFC_ERR_NO_ERROR ) return ( err );
...
static int afc_fileops_copy_internal_copy ( FileOperations * fo )
{
	...
	if ( fo->update_funct ) quit = fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_FILENAME, fo->foc->source, fo->update_info );
	if ( fo->update_funct ) quit = fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_SIZE, (void *) st.st_size, fo->update_info );
	...
	return ( AFC_LOG ( AFC_LOG_ERROR, AFC_FILEOPS_COPY_ERR_COPYING, strerror ( errno ), fo->foc->dest ) );
	    ...
	    if ( fo->update_funct ) quit =  fo->update_funct ( AFC_FILEOPS_UPDATE_MODE_POSITION, ( void * ) ( long ) size, fo->update_info );
		}
	}
	...
	if ( afc_fileops_copy_internal_set_file_stat ( fo, &st ) != AFC_ERR_NO_ERROR ) return ( AFC_FILEOPS_COPY_ERR_COPYING );
...
static int afc_fileops_internal_physical_move ( FileOperations * fo, char * source, char * dest )
{
	...
	if ( S_ISDIR (st.st_mode ) )
	{
		if ( ( err = afc_fileops_mkdir ( fo, dest ) ) != AFC_ERR_NO_ERROR ) return ( err );

	  if ( ( err = afc_fileops_internal_scan_dir ( fo, source, afc_fileops_internal_move_file, afc_fileops_internal_move_dir, NULL, dest ) ) != AFC_ERR_NO_ERROR ) return ( err );
	
		if ( ( err = afc_fileops_del ( fo, source ) ) != AFC_ERR_NO_ERROR ) return ( err );
	} else {
		...
		// Perform the copy
	  if ( ( err = afc_fileops_copy_internal_copy ( fo ) ) != AFC_ERR_NO_ERROR ) return ( AFC_FILEOPS_COPY_ERR_COPYING );
