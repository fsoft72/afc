...
Threader *afc_threader_new()
{
	...
	if (t == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "Threader", NULL);
	...
	if ((t->thread_stack = afc_array_new()) == NULL)
...
int _afc_threader_delete(Threader *t)
{
	...
	if ((afc_res = afc_threader_clear(t)) != AFC_ERR_NO_ERROR)
		return (afc_res);

	afc_dictionary_delete(t->threads);
	afc_dictionary_delete(t->mutex);
	afc_array_delete(t->thread_stack);
...
int afc_threader_clear(Threader *t)
{
	...
	afc_dprintf("%s: 6\n", __FUNCTION__);
	if (t->thread_stack)
		afc_array_clear(t->thread_stack);
...
int afc_threader_add(Threader *th, const char *name, ThreaderFunc func, void *info)
{
	...
	if (afc_dictionary_has_key(th->threads, name))
		return (AFC_ERR_NO_ERROR);
	...
	afc_array_add(th->thread_stack, td, AFC_ARRAY_ADD_TAIL);
...
int afc_threader_wait(Threader *th)
{
	...
	td = afc_array_last(th->thread_stack);
	while (td != NULL)
	{
		...
		td = afc_array_prev(th->thread_stack);
	}
...
int afc_threader_thread_lock(ThreaderData *td, char *lock_name, short wait)
{
	...
	if (res == 0)
		afc_array_add(td->locks, mutex, AFC_ARRAY_ADD_TAIL);
...
static ThreaderData *afc_threader_internal_data_new(Threader *th, void *info)
{
	...
	if ((td->locks = afc_array_new()) == NULL)
	{
...
static int afc_threader_internal_data_clear(ThreaderData *td)
{
	...
	if (td->locks)
	{
		lock = afc_array_first(td->locks);
		while (lock)
		{
			pthread_mutex_unlock(lock);
			lock = afc_array_next(td->locks);
		}

		afc_array_clear(td->locks);
	}
...
static int afc_threader_internal_data_del_lock(ThreaderData *td, pthread_mutex_t *lock)
{
	...
	ilock = afc_array_first(td->locks);
	while (ilock)
	{
		if (lock == ilock)
		{
			afc_array_del(td->locks);
			return (AFC_ERR_NO_ERROR);
		}

		ilock = afc_array_next(td->locks);
	}
