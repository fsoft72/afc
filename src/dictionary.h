...
enum {	AFC_DICTIONARY_ERR_HASHING = AFC_DICTIONARY_BASE + 1,  	/* Ths HashMaster class reported an error  */
	AFC_DICTIONARY_ERR_NOT_FOUND };		 		/* Requesteq key cannot be found           */
...
#define afc_dictionary_delete(dict)	if ( dict ) { _afc_dictionary_delete ( dict ); dict = NULL; }
...
#define afc_dictionary_num_items(d)	( d ? afc_array_master_len ( d->hash->am ) : 0 )
#define afc_dictionary_len(d)		( d ? afc_array_master_len ( d->hash->am ) : 0 )
...
#define afc_dictionary_before_first(d)	( d ? afc_array_master_before_first ( d->hash->am ) : AFC_ERR_NULL_POINTER )
...
#define afc_dictionary_set_clear_func(d,func)	if ( d ) { d->func_clear = func; }
