...
DynamicClass *afc_dynamic_class_new()
{
	...
	if (dc == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "Dynamic Class", NULL);

	dc->magic = AFC_DYNAMIC_CLASS_MAGIC;

	if ((dc->methods = afc_dictionary_new()) == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "methods", NULL);

	afc_dictionary_set_clear_func(dc->methods, afc_dynamic_class_internal_clear_method);

	if ((dc->args = afc_array_new()) == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "args", NULL);
...
int afc_dynamic_class_clear(DynamicClass *dc)
{
	...
	if (dc->magic != AFC_DYNAMIC_CLASS_MAGIC)
		return (AFC_ERR_INVALID_POINTER);

	/* Custom Clean-up code should go here */
	if (dc->vars)
		afc_dynamic_class_internal_clear_vars(dc, dc->vars);

	/* ----------------------------------  */

	if (dc->vars)
		afc_dictionary_clear(dc->vars);

	// afc_dynamic_class_internal_clear_methods ( dc );
	if (dc->methods)
		afc_dictionary_clear(dc->methods);

	if (dc->private_vars)
		afc_dictionary_clear(dc->private_vars);
...
void *afc_dynamic_class_get_var(DynamicClass *dc, char *name)
{
	...
	if ((dc == NULL) || (dc->vars == NULL))
		return (NULL);

	if ((var = afc_dictionary_get_default(dc->vars, name, NULL)) == NULL)
		return (NULL);
...
static int afc_dynamic_class_internal_parse_args(DynamicClass *dc, va_list args)
{
	...
	afc_array_clear(dc->args);

	while ((v = va_arg(args, void *)) != AFC_DYNAMIC_CLASS_ARG_END)
	{
		afc_array_add(dc->args, v, AFC_ARRAY_ADD_TAIL);
	}

	if (dc->add_arg_end)
		afc_array_add(dc->args, AFC_DYNAMIC_CLASS_ARG_END, AFC_ARRAY_ADD_TAIL);
...
int test_func(DynamicClass *dc)
{
	...
	v = (int)afc_array_first(dc->args);

	while (v != (int)AFC_DYNAMIC_CLASS_ARG_END)
	{
		printf("Arg: %d - %d\n", t++, v);

		v = (int)afc_array_next(dc->args);
	}
