...
#define AFC_READARGS_MAX_FIELD_NAME 30
  ...
  enum
  {
    AFC_READARGS_ERR_MISSING_KEYWORD = 1,
  ...
  struct afc_readargs
  {
    ...
    struct afc_string_list *global_split;
    struct afc_string_list *local_split;
...
#define afc_readargs_delete(ra) \
  if (ra)                       \
  {                             \
    _afc_readargs_delete(ra);   \
    ra = NULL;                  \
  }
