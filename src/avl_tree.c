...
AVLTree *afc_avl_tree_new(void)
{
	...
	if ((avl = afc_malloc(sizeof(AVLTree))) == NULL)
		RAISE_FAST(AFC_ERR_NO_MEMORY, "avl tree");
...
AVLNode *_afc_avl_tree_find_node(AVLTree *avl, AVLNode *node, void *x)
{
	...
	if (v < 0)
		return (_afc_avl_tree_find_node(avl, node->left, x));
	if (v > 0)
		return (_afc_avl_tree_find_node(avl, node->right, x));
...
AVLNode *_afc_avl_tree_insert(AVLTree *tree, AVLNode *node, void *key, void *val)
{
	...
	if (node == NULL)
	{
		...
		if ((node = afc_malloc(sizeof(AVLNode))) == NULL)
			RAISE_FAST(AFC_ERR_NO_MEMORY, "node");
