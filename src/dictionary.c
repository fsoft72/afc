...
TRY ( Dictionary * )
	...
	if ( dictionary == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "dictionary", NULL );

	dictionary->magic = AFC_DICTIONARY_MAGIC;

	if ( ( dictionary->hash = afc_hash_master_new () ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "hash", NULL );
...
int _afc_dictionary_delete ( struct afc_dictionary * dictionary ) 
{
	...
	if ( ( afc_res = afc_dictionary_clear ( dictionary ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );
...
int afc_dictionary_clear ( struct afc_dictionary * dictionary ) 
{
	...
	if ( dictionary == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	if ( dictionary->magic != AFC_DICTIONARY_MAGIC ) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );

	if ( dictionary->hash )
	{
		afc_array_master_before_first ( dictionary->hash->am );  // Move through all ArrayMaster items
	  	while ( ( hd = afc_array_master_next ( dictionary->hash->am ) ) )
	  	{
			ddata = hd->data;					// Get the DictionaryData inside

			if ( dictionary->func_clear ) dictionary->func_clear ( ddata->value );
...
int afc_dictionary_set ( Dictionary * dict, const char * key, void * data )
{
	...
	if ( dict == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	if ( dict->magic != AFC_DICTIONARY_MAGIC ) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );
	...
	if ( ddata == NULL )
	{
		if ( data == NULL ) return ( AFC_ERR_NO_ERROR );		// If the key does not exists and data is NULL, 
		...
		if ( ddata == NULL ) return ( AFC_LOG_FAST_INFO ( AFC_ERR_NO_MEMORY, "DictionaryData" ) );
		...
		if ( dict->func_clear ) dict->func_clear ( ddata->value );	
		ddata->value = NULL;
	}
...
{
	HashData * hmdata = ( HashData * ) afc_array_master_first ( dict->hash->am ); 

	if ( hmdata == NULL ) return ( NULL );
...
{
	HashData * hmdata = ( HashData * ) afc_array_master_next ( dict->hash->am ); 
...
{
	HashData * hmdata = ( HashData * ) afc_array_master_prev ( dict->hash->am ); 
...
{
	if ( dict->curr_data  == NULL ) return ( AFC_ERR_NO_ERROR );  // If no item is set, simply exit

	if ( dict->func_clear ) dict->func_clear ( dict->curr_data->value );	// Clear the item from memory
	...
	if ( dict->curr_data == NULL ) return ( NULL ); // Check against NULL
...
int afc_dictionary_del_item ( Dictionary * dict, const char * key )
{
	...
	if ( ddata == NULL ) return ( AFC_LOG ( AFC_LOG_WARNING, AFC_DICTIONARY_ERR_NOT_FOUND, "Not found in dictionary", key ) );
...
@node afc_dictionary_len
	  ...
	  DESCRIPTION: Use this function to get the number of keys present in the Dictionary.
	     ...
	     SEE ALSO: - afc_dictionary_first()
	               - afc_dictionary_next()
	               - afc_dictionary_prev()
		       - afc_array_master_len()
@endnode
*/
/*
unsigned long int afc_dictionary_len ( Dictionary * dict )
{
	return ( afc_array_master_len ( dict->hash->am ) );
}
*/
// }}}
...
{
	if ( afc_dictionary_internal_find ( dict, key ) != NULL ) return ( TRUE );
...
char * afc_dictionary_find_key ( Dictionary * dict, void * data )
{
	...
	while ( val )
	{
		if ( val == data ) return ( dict->curr_data->key );
...
int afc_dictionary_for_each ( Dictionary * dict, int ( *func ) ( Dictionary * am, int pos, void * v, void * info ), void * info )
{
	...
	{
		ddata = hd->data;
		if ( ( res = func ( dict, t ++, ddata->value, info ) ) != AFC_ERR_NO_ERROR ) return ( res );
