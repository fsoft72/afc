...
{
	if ( b64 == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );

	if ( b64->file_in ) 	afc_string_delete ( b64->file_in );
	if ( b64->file_out ) 	afc_string_delete ( b64->file_out );
...
int afc_base64_encode ( Base64 * b64, int first_tag, ... )
{
	...
	if ( ( b64->mem_in  == NULL ) && ( b64->file_in  == NULL ) ) return ( AFC_ERR_NO_ERROR );
	if ( ( b64->mem_out == NULL ) && ( b64->file_out == NULL ) ) return ( AFC_ERR_NO_ERROR );
	...
	if ( b64->fout ) fclose ( b64->fout );
	if ( b64->fin )  fclose ( b64->fin );
...
int afc_base64_fwrite ( Base64 * b64, const char * fname, int what )
{
	...
	// TODO: error messages
	if ( ( what == AFC_BASE64_IN )  && ( b64->mem_in  == NULL ) ) return ( AFC_ERR_NO_ERROR );
	if ( ( what == AFC_BASE64_OUT ) && ( b64->mem_out == NULL ) ) return ( AFC_ERR_NO_ERROR );
...
static int afc_base64_internal_inchar ( Base64 * b64 )
{
	if ( b64->iocp >= b64->size ) if ( afc_base64_internal_inbuf ( b64 ) != AFC_ERR_NO_ERROR ) 
	{
		b64->at_eof = TRUE;
		return EOF;
	}
...
static int afc_base64_internal_encode ( Base64 * b64 )
{
    	...
    	for ( i = 0; i < 26; i++ ) 
	{
    	...
    	for (i = 0; i < 10; i++) b64->dtable [ 52 + i ] = '0' + i;
    	...
    	while ( !quit ) 
	{
		...
		if (n > 0) 
		{
	    		...
	    		if ( n < 3 ) 
			{
	    		...
	    		for ( i = 0; i < 4; i++ ) afc_base64_internal_ochar ( b64, ogroup [ i ] );
		}
    	}
...
static int afc_base64_internal_decode ( Base64 * b64 )
{
	...
	int c;
    	...
    	for ( i = 0;   i < 255;  i++ ) b64->dtable [ i ] = 0x80;
    	for ( i = 'A'; i <= 'Z'; i++ ) b64->dtable [ i ] = 0  + ( i - 'A' );
    	for ( i = 'a'; i <= 'z'; i++ ) b64->dtable [ i ] = 26 + ( i - 'a' );
    	for ( i = '0'; i <= '9'; i++ ) b64->dtable [ i ] = 52 + ( i - '0' );
    	...
    	while (TRUE) 
	{
		...
		if ( i < 3 ) return ( AFC_ERR_NO_ERROR );
    	}
}
...
static int afc_base64_internal_write ( Base64 * b64, void * mem, int size )
{
	if ( b64->mem_out )
	{
		if ( b64->mem_out_pos >= b64->mem_out + b64->mem_out_size ) return ( AFC_BASE64_ERR_OUT_OF_MEM );	// This may be redundant
		if ( b64->mem_out_pos + size > b64->mem_out + b64->mem_out_size ) return ( AFC_BASE64_ERR_OUT_OF_MEM );
...
static int afc_base64_internal_write_char ( Base64 * b64, int c )
{
	...
	{
		if ( b64->mem_out_pos >= b64->mem_out + b64->mem_out_size ) return ( AFC_BASE64_ERR_OUT_OF_MEM );	// This may be redundant
		if ( b64->mem_out_pos + 1 > b64->mem_out + b64->mem_out_size ) return ( AFC_BASE64_ERR_OUT_OF_MEM );

		*b64->mem_out_pos  =  ( unsigned char ) c;
		b64->mem_out_pos += 1;
	} else
		if ( putc ( ( (unsigned char) c ), b64->fout ) == EOF ) exit(1);
