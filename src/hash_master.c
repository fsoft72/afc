...
TRY ( HashMaster * )
	...
	if ( hash_master == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "hash", NULL );

	hash_master->magic = AFC_HASH_MASTER_MAGIC;

	if ( ( hash_master->am = afc_array_master_new () ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "am", NULL );
...
int _afc_hash_master_delete ( struct afc_hash_master * hash_master ) 
{
	...
	if ( ( afc_res = afc_hash_master_clear ( hash_master ) ) != AFC_ERR_NO_ERROR ) return afc_res;

	afc_array_master_delete ( hash_master->am );
...
int afc_hash_master_clear ( struct afc_hash_master * hash_master ) 
{
	...
	if ( hash_master == NULL ) return AFC_LOG_FAST ( AFC_ERR_NULL_POINTER );
 
	if ( hash_master->magic != AFC_HASH_MASTER_MAGIC ) return AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER );

	if ( hash_master->am )
	{
		hd = ( HashData * ) afc_array_master_first ( hash_master->am );

		while ( hd )
		{
			if ( hash_master->func_clear ) hash_master->func_clear ( hash_master, hd->data );

			afc_free ( hd );
			hd = ( HashData * ) afc_array_master_next ( hash_master->am );
		}
		
		afc_array_master_clear ( hash_master->am );
	}
...
int afc_hash_master_add ( HashMaster * hm, unsigned long int hash_value, void * data )
{
	...
	if ( hd == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NO_MEMORY ) );

	hd->hash_value = hash_value;
	hd->data       = data;

	afc_array_master_add ( hm->am, hd, AFC_ARRAY_MASTER_ADD_TAIL );
...
void * afc_hash_master_find ( HashMaster * hm, unsigned long int hash_value )
{
	...
	// if ( afc_array_master_is_empty ( hm->am ) ) return ( NULL );
	if ( hm->am->num_items == 0 ) return ( NULL );

	if ( hm->am->is_sorted == FALSE ) afc_array_master_sort ( hm->am, afc_hash_master_internal_sort );

	// max = afc_array_master_len ( hm->am );
	max = hm->am->num_items;
	pos = max / 2;

	while ( pos!=-1 )
	{
		if ( ( hd = ( HashData * ) afc_array_master_item ( hm->am, pos ) ) == NULL ) return ( NULL );

		if ( hd->hash_value == hash_value ) return ( hd->data );
...
void * afc_hash_master_del ( HashMaster * hm )
{
	...
	if ( ( hd = afc_array_master_obj ( hm->am ) ) == NULL ) return ( NULL );  // If there is no current object, return NULL

	if ( hm->func_clear ) hm->func_clear ( hm, hd->data );		// Free associated data (if clear func exists)

	afc_free ( hd );	// Free the HashData added to the ArrayMaster

	hd = afc_array_master_del ( hm->am );
...
HashData * afc_hash_master_item ( HashMaster * hm, int item )
{
	return ( ( HashData * ) afc_array_master_item ( hm->am, item ) );
}
*/
// }}}
...
void * afc_hash_master_first ( HashMaster * hm )
{
	...
	hd = afc_array_master_first ( hm->am );
...
void * afc_hash_master_next ( HashMaster * hm )
{
	...
	hd = afc_array_master_next ( hm->am );
	if ( hd == NULL ) return ( NULL );
...
void * afc_hash_master_last ( HashMaster * hm )
{
	...
	hd = afc_array_master_last ( hm->am );
	if ( hd == NULL ) return ( NULL );
...
void * afc_hash_master_prev ( HashMaster * hm )
{
	...
	hd = afc_array_master_prev ( hm->am );
	if ( hd == NULL ) return ( NULL );
...
int afc_hash_master_for_each ( HashMaster * hm, int ( *func ) ( HashMaster * am, int pos, void * v, void * info ), void * info )
{
	...
	hd = ( HashData * ) afc_array_master_first ( hm->am );
	while ( hd )
	{
		if ( ( res = func ( hm, t++, hd->data, info ) ) != AFC_ERR_NO_ERROR ) return ( res );
		hd = afc_array_master_next ( hm->am );
	}
...
{
	return ( afc_array_master_before_first ( hm->am ) );
}
*/
// }}}
