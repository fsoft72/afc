...
CGIManager *_afc_cgi_manager_new(const char *file, const char *func, const unsigned int line)
{
	...
	if (cgi_manager == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "cgi_manager", NULL);
...
int _afc_cgi_manager_delete(CGIManager *cgi_manager)
{
	...
	if ((afc_res = afc_cgi_manager_clear(cgi_manager)) != AFC_ERR_NO_ERROR)
		return (afc_res);
...
int afc_cgi_manager_clear(CGIManager *cgi)
{
	...
	if (cgi->magic != AFC_CGI_MANAGER_MAGIC)
		return (AFC_LOG_FAST(AFC_ERR_INVALID_POINTER));

	/* Custom Clean-up code should go here */

	if (cgi->headers)
		afc_cgi_manager_internal_clear_dict(cgi, cgi->headers);
	if (cgi->fields)
...
int afc_cgi_manager_get_data(CGIManager *cgi)
{
	...
	if (cgi->method == AFC_CGI_MANAGER_METHOD_GET)
		afc_cgi_manager_internal_method_get(cgi);
	else if (cgi->method == AFC_CGI_MANAGER_METHOD_POST)
		afc_cgi_manager_internal_method_post(cgi);

	if (cgi->handle_cookies)
		afc_cgi_manager_internal_get_cookies(cgi);
...
char *afc_cgi_manager_get_val(CGIManager *cgi, char *name)
{
	...
	if (s == NULL)
		s = (char *)afc_dictionary_get(cgi->headers, str); // If it is missing, then we look in the headers
...
int afc_cgi_manager_write_header(CGIManager *cgi)
{
	...
	if ((head = afc_string_new(16384)) == NULL)
...
int afc_cgi_manager_set_default_headers(CGIManager *cgi)
{
	...
	while (fields[t])
	{
		if ((s = getenv(fields[t])))
			if (strlen(s))
				afc_dictionary_set(cgi->headers, fields[t], afc_string_dup(s));
...
static int afc_cgi_manager_internal_get_headers(CGIManager *cgi)
{
	...
	if (strcasecmp(s, "GET") == 0)
		cgi->method = AFC_CGI_MANAGER_METHOD_GET;
	else if (strcasecmp(s, "POST") == 0)
		cgi->method = AFC_CGI_MANAGER_METHOD_POST;
...
static int afc_cgi_manager_internal_method_get(CGIManager *cgi)
{
	...
	if ((s = afc_cgi_manager_get_val(cgi, "QUERY_STRING")) == NULL)
		return (AFC_ERR_NO_ERROR);
...
static int afc_cgi_manager_internal_add_key(CGIManager *cgi, char *keyval, int mode)
{
	...
	if (mode == AFC_CGI_MANAGER_MODE_FORM)
		dict = cgi->fields;
...
static int afc_cgi_manager_internal_clear_dict(CGIManager *cgi, Dictionary *dict)
{
	...
	while ((val = afc_dictionary_next(dict)))
		afc_string_delete(val);
