...
TRY ( NodeMaster * )
	...
	if ( nm == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "nm", NULL );
	nm->magic = AFC_NODEMASTER_MAGIC;

	if ( ( nm->lst = afc_malloc ( sizeof ( struct List ) ) ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "lst", NULL );
...
int _afc_nodemaster_delete(NodeMaster * nm)
{
	...
	if ( ( res = afc_nodemaster_clear(nm) ) != AFC_ERR_NO_ERROR ) return ( res );
...
void * afc_nodemaster_add(NodeMaster * nm, void * s, unsigned long mode)
{
	 ...
	 if ((mode == AFC_NODEMASTER_ADD_HERE) && (!nm->pos || IsListEmpty(nm->lst))) mode = AFC_NODEMASTER_ADD_TAIL;
...
void * afc_nodemaster_del(NodeMaster * nm)
{
	 ...
	 if (IsListEmpty(nm->lst)) return (NULL);

	 if (nm->pos)
	 {
		if ( nm->func_clear ) nm->func_clear ( nm->pos->ln_Name );
...
void * afc_nodemaster_next(NodeMaster * nm)
{
	if ( nm->before_first ) return ( afc_nodemaster_first ( nm ) );
	if ( IsListEmpty ( nm->lst ) ) return ( NULL );
...
int afc_nodemaster_clear(NodeMaster * nm)
{
	 ...
	 if ( nm == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	 if ( nm->magic != AFC_NODEMASTER_MAGIC ) return ( AFC_LOG_FAST ( AFC_ERR_INVALID_POINTER ) );

	 nm->is_sorted=FALSE;

	 if (IsListEmpty(nm->lst)) return ( AFC_ERR_NO_ERROR );

	 w = nm->lst->lh_Head;

	 while ( (n = w->ln_Succ) )
	 {
			if ( nm->func_clear ) nm->func_clear ( n->ln_Name );
			Remove(w);
			afc_free(w);
			w = n;
	 }
...
void * afc_nodemaster_item(NodeMaster * nm, unsigned long n)
{
	 ...
	 if (n != nm->npos)
	 {
			...
			if (n > nm->num) return (afc_nodemaster_last(nm));
	 ...
	 if (n > nm->npos)
	 {
			for (t = s; t < n; t++) afc_nodemaster_next(nm);
	 }
	 else
	 {
			for (t = s - 1; t >= n; t--) afc_nodemaster_prev(nm);
	 }
			}
	 }
...
{
	 if (IsListEmpty(nm->lst)) return (NULL);
	 if (nm->is_sorted) return(afc_nodemaster_first(nm));
...
struct Node * * afc_nodemaster_create_array(NodeMaster * nm)
{
	...
	if (afc_nodemaster_is_empty(nm)) return(NULL);

	if (nm->array) afc_free(nm->array);
...
void * afc_nodemaster_ultra_sort(NodeMaster * nm, int (*comp)( const void *, const void * ) )
{
	...
	if ( nm->is_array_valid == FALSE ) afc_nodemaster_create_array ( nm );
...
long afc_nodemaster_for_each(NodeMaster * nm, long (*funct) (NodeMaster * nm, void *, void *), void * info )
{
	...
	while ( data )
	{
		if ( ( result = funct ( nm, data, info ) ) != AFC_ERR_NO_ERROR ) return ( result );
...
static void afc_nodemaster_internal_quick_sort(NodeMaster * nm, unsigned long inf, unsigned long sup, signed long (*comp)(void *, void *, void *), void * info)
{
	 ...
	 if (inf < sup)
	 {
			...
			if ((signed long)inf < (mid - 1)) afc_nodemaster_internal_quick_sort(nm, inf,mid - 1,comp,info);
			if ((mid + 1) < (signed long)sup) afc_nodemaster_internal_quick_sort(nm, mid + 1,sup,comp,info);
	 }
}
...
static void afc_nodemaster_internal_fast_quick_sort(NodeMaster * nm, unsigned long inf, unsigned long sup, signed long (*comp)(void *, void *, void *), void * info)
{
	 ...
	 if (inf < sup)
	 {
			...
			if ((signed long)inf < (mid - 1)) afc_nodemaster_internal_fast_quick_sort(nm, inf,mid - 1,comp,info);
			if ((mid + 1) < (signed long)sup) afc_nodemaster_internal_fast_quick_sort(nm, mid + 1,sup,comp,info);
	 }
}
