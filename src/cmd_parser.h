...
enum { 	AFC_CMD_PARSER_ERR_NO_SCRIPT = AFC_CMD_PARSER_BASE + 1,		/* No string has been passed, so there's nothing to do */
...
/* type of token */
enum {	AFC_CMD_PARSER_TOKEN_OPEN = 1,
...
*************************/
#define afc_cmd_parser_delete(cmd)	if ( cmd ) { _afc_cmd_parser_delete ( cmd ); cmd = NULL; }
