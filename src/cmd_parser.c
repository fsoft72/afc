...
int _afc_cmd_parser_delete ( CommandParser * cmdparser ) 
{
	...
	if ( ( afc_res = afc_cmd_parser_clear ( cmdparser ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );


	/* NOTE: any class contained in afc_cmd_parser should be deleted here */

	if ( cmdparser->callbacks ) afc_nodemaster_delete ( cmdparser->callbacks );
	if ( cmdparser->classes ) afc_dictionary_delete ( cmdparser->classes );
	if ( cmdparser->stack ) afc_stringnode_delete ( cmdparser->stack );
	if ( cmdparser->token ) afc_cmd_parser_internal_token_delete ( cmdparser->token );
	if ( cmdparser->rdargs ) afc_readargs_delete ( cmdparser->rdargs );
	if ( cmdparser->buffer ) afc_string_delete ( cmdparser->buffer );
	if ( cmdparser->default_template ) afc_string_delete ( cmdparser->default_template );
	if ( cmdparser->builtins ) afc_dictionary_delete ( cmdparser->builtins );
	if ( cmdparser->functions ) afc_dictionary_delete ( cmdparser->functions );
...
int afc_cmd_parser_clear ( CommandParser * cmdparser ) 
{
	...
	if ( cmdparser == NULL ) return ( AFC_ERR_NULL_POINTER );
 
	if ( cmdparser->magic != AFC_CMD_PARSER_MAGIC ) return ( AFC_ERR_INVALID_POINTER );
	...
	if ( cmdparser->stack ) afc_stringnode_clear ( cmdparser->stack );
	if ( cmdparser->token ) afc_cmd_parser_internal_token_clear ( cmdparser->token );
	if ( cmdparser->rdargs ) afc_readargs_clear ( cmdparser->rdargs );

	if ( cmdparser->default_template ) afc_string_clear ( cmdparser->default_template );

	if ( cmdparser->functions ) afc_dictionary_clear ( cmdparser->functions );
...
int afc_cmd_parser_add_callback ( CommandParser * cmdparser, const char * name, void ( * scBack ), void ( * ecBack ), const char * templ )
{
	...
	if ( cmdparser == NULL ) return ( AFC_ERR_NULL_POINTER );
	if ( scBack == NULL ) return ( AFC_ERR_NULL_POINTER );
	if ( name == NULL ) return ( AFC_ERR_NULL_POINTER );
...
int afc_cmd_parser_parse_string ( CommandParser * cmdparser, const char * script, void * userdata )
{
			...
			if ( cmdparser->token->type == AFC_CMD_PARSER_TOKEN_OPEN ) {
				...
				while ( ( cb != NULL ) && ( found == false ) ) {
					...
					if ( strcmp ( cb->name, cmdparser->token->name ) == 0 ) {
						...
						if ( quit == AFC_ERR_NO_ERROR ) {
							if ( cb->start_fun != NULL ) quit = afc_dynamic_class_execute ( dyn, "open_callback", cmdparser->userdata, AFC_DYNAMIC_CLASS_ARG_END );
						}
				...
				while ( ( cb != NULL ) && ( found == false ) ) {
					if ( strcmp ( cb->name, cmdparser->token->name ) == 0 ) {
						...
						if ( cb->end_fun != NULL ) quit = afc_dynamic_class_execute ( dyn, "close_callback", cmdparser->userdata, AFC_DYNAMIC_CLASS_ARG_END );
						found = true;
					}
					cb = ( CommandParserCallback * ) afc_nodemaster_next ( cmdparser->callbacks );
				} /* while cb != NULL */
	...
	if ( myscript ) afc_string_delete ( myscript );
...
int afc_cmd_parser_add_commands ( CommandParser * cmdparser, DynamicClassMaster * dynmast )
{
	...
	if ( ( cmdparser == NULL ) || ( dynmast == NULL ) ) return AFC_LOG_FAST ( AFC_ERR_NULL_POINTER );
	...
	while ( ( foo != NULL ) && ( afc_res == AFC_ERR_NO_ERROR ) ) {
		...
		if ( ( dyn = afc_dynamic_class_master_new_instance ( dynmast, class_name ) ) != NULL ) {
			...
			else AFC_LOG_FAST_INFO ( afc_res, "plugin method get_template not found" );
...
{
	if ( ( cmdparser == NULL ) || ( name == NULL ) ) return ( AFC_ERR_NULL_POINTER );
...
static int afc_cmd_parser_internal_token_delete ( struct afc_cmd_parser_token * token )
{
	...
	if ( ( afc_res = afc_cmd_parser_internal_token_clear ( token ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );

	if ( token->name ) afc_string_delete ( token->name );
	afc_free ( token );
...
static int afc_cmd_parser_internal_token_clear ( struct afc_cmd_parser_token * token )
{
	if ( token == NULL ) return ( AFC_ERR_NULL_POINTER );
...
static int afc_cmd_parser_internal_token_set_name ( struct afc_cmd_parser_token * token, char * name )
{
	if ( token == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	if ( name == NULL ) return (  AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
...
static int afc_cmd_parser_internal_del_callback ( CommandParser * cmdparser, CommandParserCallback * cb )
{
	if ( cb == NULL ) return ( AFC_ERR_NULL_POINTER );

	if ( cb->name ) afc_string_delete ( cb->name );
	if ( cb->args_template ) afc_string_delete ( cb->args_template );
	afc_free ( cb );
...
static int afc_cmd_parser_internal_get_next_token ( CommandParser * cmdparser, char ** script )
{
	...
	if ( cmdparser->skip_block == 1 ) afc_cmd_parser_internal_goto_block_end ( cmdparser, script );
...
/* DEBUG */
		...
		else *script = index ( data, '\0' );
	}

	if ( ( ket == NULL ) && ( bra == NULL ) ) *script = index ( data, '\0' );
...
static int afc_cmd_parser_internal_get_token_args ( CommandParser * cmdparser, char ** script, char * template )
{
	...
	if ( cmdparser == NULL ) return ( AFC_ERR_NULL_POINTER );
	if ( template == NULL ) return ( AFC_ERR_NULL_POINTER );
	...
	if ( bra != NULL ) {
		if ( ( ket != NULL ) && ( ket < bra ) ) the_end = ket;
		else the_end = bra;
	}
	else {
		if ( ket != NULL ) the_end = ket;
		else the_end = index ( data, '\0' );
	}
...
/* DEBUG */
	...
	if ( ( afc_res = afc_readargs_parse ( cmdparser->rdargs, template, data ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );
...
static int afc_cmd_parser_internal_string_purge ( CommandParser * cmdparser, char * s )
{
	...
	if ( s == NULL ) return ( AFC_ERR_NULL_POINTER );
...
static int afc_cmd_parser_internal_set_skip ( CommandParser * cmdparser, int howmany )
{
	if ( cmdparser == NULL ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
...
static int afc_cmd_parser_internal_keyword_if ( CommandParser * cmdparser, char * script, NodeMaster * args )
{
	...
	if ( ( cmdparser == NULL ) || ( args == NULL ) ) return ( AFC_LOG_FAST ( AFC_ERR_NULL_POINTER ) );
	...
	if ( ( func_name = ( char * ) afc_nodemaster_first ( args ) ) != NULL ) 
	{
		...
		// FIXED: non liberavi func_name quando avevi già "scaricato" la stringa dal nodemaster
