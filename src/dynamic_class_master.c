...
TRY ( DynamicClassMaster * )
	...
	if ( dcm == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "dcm", NULL );

	dcm->magic = AFC_DYNAMIC_CLASS_MASTER_MAGIC;

	if ( ( dcm->classes = afc_dictionary_new () ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "classes", NULL );
	afc_dictionary_set_clear_func ( dcm->classes, afc_dynamic_class_master_internal_clear_class );

	if ( ( dcm->instances = afc_hash_master_new () ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "instances", NULL );
...
int _afc_dynamic_class_master_delete ( DynamicClassMaster * dcm ) 
{
	...
	if ( ( afc_res = afc_dynamic_class_master_clear ( dcm ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );
...
int afc_dynamic_class_master_clear ( DynamicClassMaster * dcm ) 
{
	...
	if ( dcm == NULL ) return ( AFC_ERR_NULL_POINTER );
 
	if ( dcm->magic != AFC_DYNAMIC_CLASS_MASTER_MAGIC ) return ( AFC_ERR_INVALID_POINTER );
...
char * afc_dynamic_class_master_get_info ( DynamicClassMaster * dcm, char * class_name, int info_id )
{
	...
	if ( data->get_info == NULL ) return ( NULL );
...
static int afc_dynamic_class_master_internal_clear_class ( void * c )
{
	...
	if ( data == NULL ) return ( AFC_ERR_NO_ERROR );

	if ( data->dl_handler ) dlclose ( data->dl_handler );
...
static int afc_dynamic_class_master_internal_clear_instance ( HashMaster * hm, void * d )
{
	...
	if ( idata == NULL ) return ( AFC_ERR_NO_ERROR );
