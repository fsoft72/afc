...
TRY ( DBIManager * )
	...
	if ( dbi == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "dbi", NULL );
...
int _afc_dbi_manager_delete ( DBIManager * dbi )
{
  	...
  	if ( ( afc_res = afc_dbi_manager_clear ( dbi ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );
...
{
  	if ( dbi_manager == NULL ) return ( AFC_ERR_NULL_POINTER );
 
  	if ( dbi_manager->magic != AFC_DBI_MANAGER_MAGIC ) return ( AFC_ERR_INVALID_POINTER );
...
DynamicClass * afc_dbi_manager_new_instance ( DBIManager * dbi, const char * class_name, const char * library_name )
{
	...
	if ( afc_dynamic_class_master_has_class ( dbi->dcm, class_name ) != AFC_ERR_NO_ERROR )
	{
		...
		if ( afc_dynamic_class_master_load ( dbi->dcm, class_name, s ) != AFC_ERR_NO_ERROR ) return ( NULL );
	}
