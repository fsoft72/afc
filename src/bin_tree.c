...
BinTree *afc_bin_tree_new(void)
{
	...
	if (bt == NULL)
		RAISE_FAST_RC(AFC_ERR_NO_MEMORY, "bin_tree", NULL);
...
int afc_bin_tree_traverse(BinTree *bt, int mode, int (*visit)(BinTree *, BinTreeNode *))
{
	switch (mode)
	{
	case AFC_BIN_TREE_MODE_INORDER:
		return (afc_bin_tree_int_inorder(bt, bt->root, visit));
	case AFC_BIN_TREE_MODE_PREORDER:
		return (afc_bin_tree_int_preorder(bt, bt->root, visit));
	case AFC_BIN_TREE_MODE_POSTORDER:
		return (afc_bin_tree_int_postorder(bt, bt->root, visit));
	}
