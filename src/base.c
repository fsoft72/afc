...
int afc_delete (AFC * afc)
{
	...
	if ( ( afc_res = afc_clear ( afc ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );

	if ( afc->tracker ) afc_mem_tracker_delete ( afc->tracker );
	afc->tracker = NULL;

	if ( afc->tmp_string ) afc_string_delete ( afc->tmp_string );
	if ( afc->last_error ) afc_string_delete ( afc->last_error );
	...
	if ( ( afc->fout ) && ( afc->fout != stderr ) ) fclose ( afc->fout );
...
{
	if ( afc == NULL ) return ( AFC_ERR_NULL_POINTER );

	if (afc->magic != AFC_MAGIC) return (AFC_ERR_INVALID_POINTER);

	if ( afc->tmp_string ) afc_string_clear ( afc->tmp_string );
	if ( afc->last_error ) afc_string_clear ( afc->last_error );
...
int afc_log (AFC * afc, int level, unsigned int error, const char *class_name, const char *funct_name, const char *descr, const char *info)
{
	...
	if ( ( descr != NULL )  && ( afc->last_error != NULL ) ) afc_string_copy ( afc->last_error, descr, ALL );

	if ( afc->fout == NULL ) return ( error );

// printf ( "ERR: Level: %d - Start: %d - Class: %s - Descr: %s\n", level, afc->start_log_level, class_name, descr );

	if ( level > AFC_LOG_CRITICAL ) return ( error );

	if ( level < afc->start_log_level ) return ( error );

	fprintf ( afc->fout, "------------------------ %s -------------------------\n", str_level [ level ] );

	if ( class_name ) fprintf ( afc->fout, "Class: %s\n", class_name );
	if ( funct_name ) fprintf ( afc->fout, "Funct: %s\n", funct_name );
	if ( descr )      fprintf ( afc->fout, "Descr: %s\n", descr );
	if ( info )       fprintf ( afc->fout, " Info: %s\n", info );

	fprintf ( afc->fout, " Code: %x\n", error );

	if ( ( level == AFC_LOG_CRITICAL ) && ( afc->log_exit_critical ) ) exit ( 1 );
...
{
	if ( level > afc->debug_level ) return ( AFC_ERR_NO_ERROR );
	if ( afc->fout == NULL ) return ( AFC_ERR_NO_ERROR );
...
int afc_set_tag (AFC * afc, int tag, void *val)
{
	switch (tag) 
	{
		...
		case AFC_TAG_SHOW_MALLOCS:
			if ( afc->tracker ) afc->tracker->show_mallocs  = ( short ) ( int ) (long) val;
			break;

		case AFC_TAG_SHOW_FREES:
			if ( afc->tracker ) afc->tracker->show_frees  = ( short ) ( int ) (long) val;
			break;
...
{
	if ( afc->tracker != NULL ) return afc->tracker;
...
int _afc_dprintf ( const char * fmt, ... )
{
	...
	if ( __internal_afc_base == NULL ) return ( AFC_ERR_NO_ERROR );

	f = __internal_afc_base->fout;

	if ( f == NULL ) return ( AFC_ERR_NO_ERROR );
...
static void afc_internal_on_exit ( void )
{
	if ( __internal_afc_base ) afc_delete ( __internal_afc_base );
}
// }}}
