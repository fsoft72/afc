...
#define afc_hash_master_delete(hm)	if ( hm ) { _afc_hash_master_delete ( hm ); hm = NULL; }
...
#define afc_hash_master_set_clear_func(hm,func) 	if ( hm ) { hm->func_clear = func; }
#define afc_hash_master_set_custom_sort(hm,func) 	if ( ( hm )  && ( hm->am ) ) { hm->am->custom_sort = func; }
#define afc_hash_master_before_first(hm)		( hm ? afc_array_master_before_first ( hm->am ) : AFC_ERR_NULL_POINTER )
#define afc_hash_master_is_empty(hm)			( hm ? afc_array_master_is_empty ( hm->am ) : TRUE )
#define afc_hash_master_is_first(hm)			( hm ? afc_array_master_is_first ( hm->am ) : FALSE )
#define afc_hash_master_is_last(hm)			( hm ? afc_array_master_is_last ( hm->am ) : FALSE )
#define afc_hash_master_len(hm)				( hm ? afc_array_master_len ( hm->am ) : -1 )
