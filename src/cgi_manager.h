...
#define AFC_CGI_MANAGER_BASE 0xb000
	...
	enum
	{
		AFC_CGI_MANAGER_ERR_NO_REQUEST = AFC_CGI_MANAGER_BASE + 1, /* Pointer passed is not of the right kind */
		AFC_CGI_MANAGER_ERR_POST_READ							   /* Error reading the POST */
	};

	enum
	{
		AFC_CGI_MANAGER_METHOD_UNDEF = 0,
	...
	enum
	{
		AFC_CGI_MANAGER_MODE_FORM = 0,
		AFC_CGI_MANAGER_MODE_COOKIE
	};

	enum
	{
		AFC_CGI_MANAGER_TAG_DEBUG = 0,
...
#define afc_cgi_manager_delete(cgi)   \
	if (cgi)                          \
	{                                 \
		_afc_cgi_manager_delete(cgi); \
		cgi = NULL;                   \
	}
