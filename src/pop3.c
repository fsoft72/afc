...
TRY ( POP3 * )
  	...
  	if ( p == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "pop3", NULL );
  	p->magic = AFC_POP3_MAGIC;

	if ( ( p->ic  = afc_inet_client_new () ) == NULL )  RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "inet_client", NULL );
	if ( ( p->sn  = afc_stringnode_new () ) == NULL )   RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "sn", NULL );
	if ( ( p->tmp = afc_string_new ( 1024 ) ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "tmp", NULL );
	if ( ( p->buf = afc_string_new ( 1024 ) ) == NULL ) RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "buf", NULL );
	if ( ( p->msg = afc_hash_master_new () ) == NULL )  RAISE_FAST_RC ( AFC_ERR_NO_MEMORY, "msg", NULL );
...
int _afc_pop3_delete ( POP3 * p ) 
{
  	...
  	if ( ( afc_res = afc_pop3_clear ( p ) ) != AFC_ERR_NO_ERROR ) return ( afc_res );
...
{
  	if ( p == NULL ) return ( AFC_ERR_NULL_POINTER );
 
  	if ( p->magic != AFC_POP3_MAGIC ) return ( AFC_ERR_INVALID_POINTER );
	if ( p->ic ) afc_inet_client_clear ( p->ic );
	if ( p->sn ) afc_stringnode_clear ( p->sn );
	if ( p->tmp ) afc_string_clear ( p->tmp );
	if ( p->buf ) afc_string_clear ( p->buf );

	if ( p->port ) 		afc_string_copy ( p->port, "110", ALL );

	if ( p->host ) 		afc_string_delete ( p->host );
	if ( p->login ) 	afc_string_delete ( p->login );
	if ( p->passwd )	afc_string_delete ( p->passwd );
...
int afc_pop3_connect ( POP3 * p )
{
	...
	if ( ( res = afc_inet_client_open ( p->ic, p->host, atoi ( p->port ) ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
int afc_pop3_login ( POP3 * p ) 
{
	...
	if ( ( res = afc_pop3_internal_cmd ( p, p->tmp, FALSE ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
int afc_pop3_stat ( POP3 * p )
{
	...
	if ( ( res = afc_pop3_internal_cmd ( p, "STAT", FALSE ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
int afc_pop3_get_list ( POP3 * p )
{
	...
	if ( ( res = afc_pop3_internal_cmd ( p, "LIST", TRUE ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
static int afc_pop3_internal_get_response ( POP3 * p )
{
	...
	if ( ( res = afc_pop3_internal_get_line ( p ) ) != AFC_ERR_NO_ERROR ) return ( res );
	

	afc_dprintf ( "RESP: '%s'\n", p->buf );

	if ( p->buf [ 0 ] != '+' ) return ( AFC_LOG ( AFC_LOG_ERROR, AFC_POP3_ERR_PROTOCOL, "Protocol Error", p->buf ) );
...
static int afc_pop3_internal_put ( POP3 * p, const char * msg )
{
	...
	if ( ( res = afc_inet_client_send ( p->ic, msg, 0 ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
static int afc_pop3_internal_get_multi_line ( POP3 * p )
{
	...
	if ( ( res = afc_pop3_internal_get_response ( p ) ) != AFC_ERR_NO_ERROR ) return ( res );
...
static int afc_pop3_internal_cmd ( POP3 * p, const char * cmd, short multi )
{
	...
	if ( res != AFC_ERR_NO_ERROR ) return ( res );

	if ( multi ) return ( afc_pop3_internal_get_multi_line ( p ) );
...
static int afc_pop3_internal_del_msg ( HashMaster * hm, void * m )
{
	...
	if ( msg->subject ) 	afc_string_delete ( msg->subject );
	if ( msg->from ) 	afc_string_delete ( msg->from );
